...
export async function fetchContextMessages(ctx, component, args) {
    ...
    if (opts.searchOptions?.textSearch || opts.searchOptions?.vectorSearch) {
        ...
        assert(!args.contextOptions?.searchOptions?.vectorSearch || "runAction" in ctx, "You must do vector search from an action");
        if (opts.searchOptions?.vectorSearch && !args.getEmbedding) {
            throw new Error("You must provide an embedding and embeddingModel to use vector search");
        }
        const embeddingFields = opts.searchOptions?.vectorSearch
            ? await args.getEmbedding?.(text)
            : undefined;
        const searchMessages = await ctx.runAction(component.messages.searchMessages, {
