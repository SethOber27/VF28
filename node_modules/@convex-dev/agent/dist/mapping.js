import { convertToCoreMessages, coreMessageSchema, } from "ai";
import { assert } from "convex-helpers";
import { MAX_FILE_SIZE, storeFile } from "./client/files.js";
export async function serializeMessage(ctx, component, messageWithId) {
...
export async function serializeNewMessagesInStep(ctx, component, step, metadata) {
    ...
    const messages = await Promise.all((step.toolResults.length > 0
        ...
        : step.response.messages.slice(-1)).map(async (messageWithId) => {
        const { message, fileIds } = await serializeMessage(ctx, component, messageWithId);
        return {
            message,
...
export async function serializeObjectResult(ctx, component, result, metadata) {
...
export async function serializeContent(ctx, component, content) {
    ...
    const fileIds = [];
    const serialized = await Promise.all(content.map(async ({ experimental_providerMetadata, ...rest }) => {
        ...
        switch (part.type) {
            ...
            case "file": {
                ...
                if (data instanceof ArrayBuffer && data.byteLength > MAX_FILE_SIZE) {
                    const { file } = await storeFile(ctx, component, new Blob([data], { type: part.mimeType }));
                    data = file.url;
                    fileIds.push(file.fileId);
                }
                return { ...part, data };
...
export function deserializeUrl(urlOrString) {
...
export function toUIFilePart(part) {
    const dataOrUrl = serializeDataOrUrl(part.type === "image" ? part.image : part.data);
    return {
        type: "file",
        data: dataOrUrl instanceof ArrayBuffer ? encodeBase64(dataOrUrl) : dataOrUrl,
...
export function promptOrMessagesToCoreMessages(args) {
    const messages = [];
    assert(args.prompt || args.messages || args.promptMessageId, "messages or prompt or promptMessageId is required");
    if (args.messages) {
        if (args.messages.some((m) => typeof m === "object" &&
            m !== null &&
            (m.role === "data" || // UI-only role
                "toolInvocations" in m || // UI-specific field
                "parts" in m || // UI-specific field
                "experimental_attachments" in m))) {
            messages.push(...convertToCoreMessages(args.messages));
        }
        else {
