...
async function abortById(ctx, args) {
    ...
    if (stream.state.kind !== "streaming") {
        console.warn(`Stream trying to abort but not currently streaming (${stream.state.kind}): ${args.streamId}`);
        return false;
    }
...
export const finish = mutation({
    ...
    handler: async (ctx, args) => {
        ...
        if (stream.state.kind !== "streaming") {
            console.warn(`Stream trying to finish but not currently streaming: ${args.streamId}`);
            return;
        }
        await cleanupTimeoutFn(ctx, stream);
