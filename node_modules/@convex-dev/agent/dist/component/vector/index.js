...
import { getVectorTableName, vEmbeddingsWithoutDenormalizedFields, vVectorDimension, vVectorId, } from "./tables.js";
export const paginate = query({
    ...
    handler: async (ctx, args) => {
        ...
        const vectors = await paginator(ctx.db, schema)
            ...
            .withIndex("model_table_threadId", (q) => args.table
            ? // eslint-disable-next-line @typescript-eslint/no-explicit-any
                q.eq("model", args.targetModel).eq("table", args.table)
...
export const deleteBatchForThread = mutation({
    ...
    handler: async (ctx, args) => {
        const tableName = getVectorTableName(args.vectorDimension);
        const vectors = await mergedStream(["thread", "memory"].map((table) => stream(ctx.db, schema)
            .query(tableName)
            .withIndex("model_table_threadId", (q) => q
            .eq("model", args.model)
            .eq("table", table)
            .eq("threadId", args.threadId))), ["threadId"]).paginate({
            cursor: args.cursor ?? null,
            numItems: args.limit,
            maximumRowsRead: 300,
        });
...
export const insertBatch = mutation({
    ...
    handler: async (ctx, args) => {
        return Promise.all(args.vectors.map(async ({ messageId, ...v }) => {
...
export async function insertVector(ctx, dimension, v) {
...
export function searchVectors(ctx, vector, args) {
    ...
    return ctx.vectorSearch(tableName, "vector", {
        ...
        filter: (q) => args.searchAllMessagesForUserId
...
export const updateBatch = mutation({
    ...
    returns: v.null(),
    handler: async (ctx, args) => {
        await Promise.all(args.vectors.map((embedding) => ctx.db.patch(embedding.id, {
