...
export { toUIMessages };
export function mergeDeltas(threadId, streamMessages, existingStreams, allDeltas) {
    const newStreams = [];
    ...
    for (const streamMessage of streamMessages) {
        ...
        const [newStream, messageChanged] = applyDeltasToStreamMessage(threadId, streamMessage, existing, deltas);
...
export function applyDeltasToStreamMessage(threadId, streamMessage, existing, deltas) {
    let changed = false;
    ...
    for (const delta of deltas.sort((a, b) => a.start - b.start)) {
        ...
        if (cursor !== delta.start) {
            if (cursor >= delta.end) {
                console.debug(`Got duplicate delta for stream ${delta.streamId} at ${delta.start}`);
                continue;
            }
    ...
    if (existingMessages.length > 0) {
        ...
        const newMessage = createStreamingMessage(threadId, streamMessage, parts[0], existingMessages.length);
        parts = parts.slice(1);
    ...
    for (const part of parts) {
        ...
        if (isToolRole !== (currentMessage.message.role === "tool")) {
            currentMessage = createStreamingMessage(threadId, streamMessage, part, newStream.messages.length);
        ...
        if (contentToAdd) {
            ...
            currentMessage.message.content.push(contentToAdd);
            lastContent = contentToAdd;
        }
    }
    return [newStream, true];
}
function cloneMessageAndContent(message) {
    return (message &&
        {
            ...message,
...
function getLastContent(message) {
...
export function createStreamingMessage(threadId, message, part, index) {
